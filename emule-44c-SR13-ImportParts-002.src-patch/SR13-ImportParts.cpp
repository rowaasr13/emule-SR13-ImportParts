...
int CImportPartsFileThread::Run(){
	...
	if(!f.Open(m_strFilePath, CFile::modeRead)){
		LogError(LOG_STATUSBAR, GetResString(IDS_SR13_IMPORTPARTS_ERR_CANTOPENFILE), m_strFilePath);
		*bInProgress = false;
		return false;
	}
	...
	Log(LOG_STATUSBAR, GetResString(IDS_SR13_IMPORTPARTS_IMPORTSTART), partcount+1, partcount, m_partfile->GetFileName());
